import styles from './CRUDBookFormComponent.module.css';
...
import { BookClass, QuestionClass } from '../../../Rest-APi-Client/shared-types';
import { bookApi, commentApi, questionApi } from '../../../Rest-APi-Client/client';
...
export default function CRUDBookFormComponent() {
   // get PROPS by Link-> state -> useLocation.state React-router
   const location = useLocation().state;
   let navigate = useNavigate();

   const { handleSubmit, control, formState: { errors, isValid, isDirty } } = useForm<IBookFormInputs>({
      defaultValues: {
         title: `${location?.title || ""}`,
         bookPic: `${location?.bookPic || ""}`
      },
      mode: "onChange",
   ...
   const sendSubmit = (data: IBookFormInputs, event: React.BaseSyntheticEvent<object, any, any> | undefined) => {
      if (event !== undefined) {
         event.preventDefault();
      }
      const newBook = new BookClass(
         location?.id || undefined,
         location?.ownerId || "63451e55eed4a9bffb5e21b4", //logged user if new Q
         data.title,
         `${data.bookPic === ""
      ...
      if (location !== null) {
         fetch(`http://localhost:8000/api/ExchangePage/${newBook.id}`, {
            method: 'PUT',
   ...
   const deleteBook = () => {
      bookApi.deleteById(location.id)
         .then(res => {
            navigate(-1)
         })
   }
         ...
         <Container component="main" maxWidth="md" className={styles.mainFormWrapper}
            ...
            >
               <Avatar sx={{ m: 1, bgcolor: 'lightgreen' }}>
                  <CompareArrowsIcon style={{ fontSize: "34px" }} />
               </Avatar>
               ...
               <Typography component="h1" variant="h5">
                  {
                     location !== null
                        ? ("✏️ Editing your book...")
                        : ("Add 📗 to the exchange section.")
                  }

               </Typography>
                  {!location && "Your old reading can make someone really 😊"}
               ...
               >
                  ...
                  <Controller
                     ...
                     render={({ field: { onChange, value } }) => (
                        <TextField
                           ...
                           error={errors.bookPic?.message ? true : false}
                  ...
                  >
                     {location !== null ? "Save changes" : "Add the book to the exchange shelf"}
                  </Button>
